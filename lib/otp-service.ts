...
export async function sendOTP(
  ...
  try {
    ...
    // Validate phone number format - accept 10 digits (without country code)
    // or 12 digits with country code 91
    let phoneToSend = cleanPhone;

    if (cleanPhone.length === 10) {
      // Already without country code, add 91 for WasenderAPI
      ...
      return {
        success: false,
        message:
          "Invalid phone number format. Must be 10 digits (without country code) or 12 digits with country code 91.",
    ...
    // Use the provided code or generate a 6-digit OTP
...
export async function resendOTP(
  ...
  // Send new OTP
  return sendOTP(cleanPhone, code);
}
